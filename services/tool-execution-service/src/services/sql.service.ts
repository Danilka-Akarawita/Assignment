...
export class SqlService {
  async execute(query: string, userId: number): Promise<SqlQueryResult> {
    ...
    logger.debug({ userId, queryPreview: safeQuery.slice(0, 120) }, 'Executing read-only SQL');
    ...
    return {
      columns,
      rows: limited.map((row: Record<string, unknown>) => this.serializeRow(row)),
  ...
  async getSchemaCatalog(): Promise<DatabaseSchemaCatalog> {
    ...
    return {
      ...
      pgvector: {
        note:
          'pgvector does not generate SQL from natural language. Use these operators in SELECT queries after obtaining an embedding from the knowledge service.',
        operators: [
          {
            operator: '<=>',
